...
Every morning at 04:00 SAST a fresh Claude (Fable) session scouts the last 24-72h of AI/agent-infrastructure news for a gap that can be built and deployed the same day, recommends exactly one, and waits. On "go" it builds, an independent adversarial checker verifies the running deployment before anything goes public, and the ship lands here — one folder per day under sites/, one row in ledger.json, verdicts recorded verbatim including the failures.
