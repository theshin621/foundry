...
**CLOCK STATUS at promotion (2026-08-09).** Clock-start = first fire after wired ∧ beacon armed ∧ segment locked ∧ domain attached. Standing **(updated 2026-08-09 13:14Z by the relay fire; instrument-read, not assumed)**: wired ✓ · domain attached ✓ (2026-08-08, Theshin's hand) · **beacon armed ✓ — the "IN FLIGHT" note above is superseded**: `ledger.json` row 8 `beacon-firstparty` is `status: live`, `stats_endpoint_live: true`, read live `2026-08-09T09:48:36Z` · **segment aim ✓ / lock ATTESTATION-PENDING** — a fire payload relayed Theshin's pick by number against the `briefs/2026-08-09.md` §5 shortlist (`"1,2,3,4,6"` = media · gaming · social · messaging · dev-tools, payments excluded). The pick is ACTIVE as the scouting aim; see `decisions/2026-08-09-segment-lock.md` for the full adjudication and the evidence it rests on. **The 30-day clock has NOT started, and is HELD on one word from Theshin ("clock go") rather than on the relay** — clock-start is the half whose cost does not unwind, because the day-30 terminate-or-pivot verdict inherits its premise. Each day held moves clock-start forward one day; nothing else changes. The binding 30-ship/zero-signal clause remains; whichever trips first.
