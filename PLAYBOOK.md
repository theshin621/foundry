...
STOP-CONDITION (the 30-day clock). Definitions: beacon armed = live on the four current ships + every merge thereafter. Qualified visit = JS-executed beacon hit minus known-bot UAs and ?self=1 self-traffic. Visit threshold = ≥250 qualified visits on one ship [MOD-2], spread across ≥3 distinct days, within a trailing-7-day window (pending Theshin's final sign-off of this proxy; flag it in the next Sunday triage if unconfirmed). Segment lock = scout presents the evidence-ranked segment shortlist at a Sunday triage, Theshin picks by number, locked for the window. Clock-start = first fire after: wired ∧ beacon armed ∧ segment locked ∧ domain attached (Theshin's go). Predicate: within 30 days of clock-start, ≥1 post-clock-start ship in the locked segment reaches (A) a rail-receipted first real dollar (rail approval = Theshin's judgement; the receipt = the instrument) OR (B) the visit threshold — whichever first; both instrument-read, never claimed. Build-day rule: any BOTTLENECKS cause at count ≥2 without a shipped fix blocks build days while outstanding. Day-30 evaluation: no such cause outstanding · all published ships checker-PASSed · zero exposure outside the gate set above. Miss → operator drafts the self-kill/redesign proposal for Sunday triage, Theshin decides. The binding 30-ship/zero-signal clause stays; whichever trips first. **[SUPERSEDED by Amendment METHODOLOGY v2 [V2-1]/[V2-2] — day 30 is a REVIEW, not a verdict; branches A/B are uptake evidence, not triggers.]** ≥3 consecutive dead fires → operator surfaces a clock-pause decision early.
```

</details>

**CLOCK STATUS at promotion (2026-08-09).** Clock-start = first fire after wired ∧ beacon armed ∧ segment locked ∧ domain attached. Standing **(updated 2026-08-09 13:14Z by the relay fire; instrument-read, not assumed)**: wired ✓ · domain attached ✓ (2026-08-08, Theshin's hand) · **beacon armed ✓ — the "IN FLIGHT" note above is superseded**: `ledger.json` row 8 `beacon-firstparty` is `status: live`, `stats_endpoint_live: true`, read live `2026-08-09T09:48:36Z` · **segment aim ✓ / lock ATTESTATION-PENDING** — a fire payload relayed Theshin's pick by number against the `briefs/2026-08-09.md` §5 shortlist (`"1,2,3,4,6"` = media · gaming · social · messaging · dev-tools, payments excluded). The pick is ACTIVE as the scouting aim; see `decisions/2026-08-09-segment-lock.md` for the full adjudication and the evidence it rests on. ~~**The 30-day clock has NOT started, and is HELD on one word from Theshin ("clock go") rather than on the relay**~~ — **SUPERSEDED 2026-08-09T14:52Z by the clock-go relay fire. THE CLOCK IS RUNNING: clock-start `2026-08-09T14:52:09Z`, day 30 = `2026-09-08`.** Theshin's one-tap word arrived and the fire executed the rule the 13:14Z adjudication had pre-committed verbatim. Grounds, and the objection the fire logged against its own ruling (`"clock go"` is zero-entropy corroboration — the loop published the phrase itself 98 minutes earlier), are in `decisions/2026-08-09-segment-lock.md`. Revocable in one sentence at any point: `"clock stop"` · `"clock wrong, start <date>"`. Day 30 produces a **proposal**, never an autonomous act. The binding 30-ship/zero-signal clause remains; whichever trips first.
...
### Amendment — METHODOLOGY v2, 2026-08-09 (win condition + graduation; per Theshin, live session, evening; relayed to the 15:02Z methodology fire. Full adjudication: `decisions/2026-08-09-methodology-v2.md`)

Theshin, verbatim: *"'first dollar before Bet' nope, it goes through a judge cycle, do all, 1,2,3 and 4d. i am in charge and this is the methodology. We can review uptake every 30 days. It does not cost us anything to build and ship."*

**[V2-1] WIN CONDITION — IN FORCE.** The primary operating metric is **checker-PASSed ships published per 30-day cycle** — build-and-ship cadence, not an adoption threshold. The **30-day terminate-or-pivot clause** (§STOP-CONDITION day-30 evaluation) and the **30-ship / zero-cumulative-signal self-kill** (§1, "Loop-level self-kill") are **SUPERSEDED** — recorded, not deleted — by a **30-DAY UPTAKE REVIEW**.

**[V2-2] THE 30-DAY UPTAKE REVIEW — mandatory, first on 2026-09-08.** At each 30-day mark the OPERATOR compiles per-ship uptake — qualified visits, inbound, any dollars — and Theshin plus a **founders-board-style adversarial JUDGE CYCLE** rules **graduate / continue / kill** per ship. **Nothing auto-terminates on uptake alone; nothing auto-graduates either.** The instrument is unchanged and non-negotiable: the cookieless beacon, the qualified-visit definition, MOD-2's ≥250 / ≥3 distinct days / trailing-7, per-fire d30 + kill computations, and NON-NULL instrument-read ledger signal blocks all continue exactly as built. Stop-condition branches A (rail-receipted dollar) and B (visit threshold) are retained as **uptake evidence feeding the review**, not as triggers. **KILL remains a live verdict** — a review that never kills is not a review. The ≥3-consecutive-dead-fires clock-pause proposal stays in force.

**[V2-3] GRADUATION — process ACTIVE, authority HELD.** A foundry ship graduates toward the Bet portfolio through the adversarial, uptake-informed **judge cycle**, replacing first-dollar-before-Bet **for foundry ships only**. Scope: this amends the foundry's graduation path; the **vault-level commit rule outside the foundry is untouched**. Money-touching steps (rail activation and everything downstream) remain **ALWAYS-Theshin**.
**The HOLD, visible per the payload's own instruction:** MOD-1's gate text names the predicate verbatim — *"Bet promotion (commit rule: first dollar, Theshin's hand)"* — so striking *first dollar* edits the words of a GATES clause, and **a gate change never activates from a fire payload**. Therefore: the judge cycle **runs and recommends** from now; a `graduate` verdict is a **recommendation carried to Theshin**, and the first-dollar predicate is `SUSPENDED-PENDING-LIVE-WORD`, not deleted. **One sentence from Theshin in a live session closes the hold.** No ship crosses into the Bet portfolio before that. Practical delta of the hold: near zero — promotion was always his hand.

**[V2-4] CORROBORATION NOTE (recorded against this run's own ruling).** `4d` indexes cleanly onto the relayed menu's option **d**; `1,2,3` indexes onto **nothing checkable in this repo** — the menu as described is lettered a–d and no numbered list from that session exists here. Only what the payload's body specifies is encoded, and the 1/2/3 mapping is the relay's interpretation, not a verified index. Both substantive changes stand on their own quoted words (*"it goes through a judge cycle"*, *"We can review uptake every 30 days"*).

**[V2-5] QUEUED — decision-debt, owner Theshin.** (a) `workers.dev` toggle-off after serve-verification — his click or his Chrome session. (b) GitHub repo transfer to a neutral org — **must be sequenced with PAT reissue** so the daily fire never loses push; **never attempted from a fire.**

**Reversal:** one sentence, either way. *"no, keep the 250 threshold as the win condition"* reverts [V2-1]/[V2-2]; *"drop first-dollar for foundry ships"* closes the [V2-3] hold.
...
**[SUPERSEDED by Amendment METHODOLOGY v2 [V2-1]/[V2-2] — 2026-08-09: replaced by the mandatory 30-day uptake review + judge cycle; kept for the record.]** **Loop-level self-kill (binding):** at ship #30, if cumulative signal across ALL ships is zero — no organic usage, no installs, no inbound, no revenue — the Sunday run must table a terminate-or-pivot recommendation as its lead item. The loop proposes its own death; it never rides on sunk cost.
...
### Amendment — 2026-08-09 (distribution & naming policy; per Theshin, relayed verbatim — NOT a gate change)

Theshin, live session 2026-08-09 evening: *"i dont like the fact that my full name is exposed in the github url and tweets… I would like the tweet to reflect the published cloudflare url of the working solutions. when something is pushed to github we shouldnt tweet then."* Classified as **operational output discipline, not a gate change** — nothing in THE GATES moved, no autonomy widened, and every clause below is a *restriction*. A fire payload may tighten what the loop writes into public text; it may never loosen a gate. Full record: `decisions/2026-08-09-distribution-naming-policy.md`.

1. **X-draft trigger — PUBLISHED, not pushed.** A draft is authored or refreshed **only when a ship is published** (live on `tailorfarms.com`). Never at push time, never at stage time. A push to GitHub is not a publication event and produces no draft. The OPERATOR role's "X post DRAFTED into the brief" step is now conditional on publication; a staged-but-unmerged ship gets no draft.
2. **X-draft link — one URL, the ship's own.** Every draft links **only** its `tailorfarms.com` URL. **No GitHub links, no `workers.dev` URLs, no personal-name strings — in any draft or any public-facing text, ever.** Posting remains what it always was: drafted, never posted, the Post tap always Theshin's.
3. **Public surfaces carry the foundry brand only.** `public/`, the hub, the dashboard, `SHIPPED.md` and both ledger copies are swept for the old `workers.dev` host and for personal-name strings. This is the standing GATES clause *"foundry brand, never Theshin's name"* made concrete and checkable rather than aspirational.
4. **Instrument output is never hand-edited to match policy.** `public/health.json` is written by the Actions probe. A scrub that rewrites its `base` field is a hand-written measurement and is forbidden — the same rule as the ledger's non-null signal blocks. (Caught and reverted in-run on 2026-08-09.) When a claim about the live site must change, **repoint the instrument and let it write the proof**; `health-check.yml` now probes `BASE: https://tailorfarms.com`.
5. **A scrub may not create a dead link.** Blanket host substitution across a page rewrites *repo file paths* too, silently producing 404s (five of them on 2026-08-09, caught in-run). Any naming sweep ends with a dead-link scan plus a cold local serve of `public/`, and operator links to files GitHub is the only host for stay **functional** — the label stops printing the path, the link keeps working.
6. **The GitHub identity is Theshin's decision, never the loop's action.** The loop does not rename or transfer the repo. It is decision-debt with a step list (see the decision file); at 2 consecutive triages unresolved it becomes a `BOTTLENECKS.md` entry. Build-in-public continues meanwhile — the repo stays public; the loop simply stops advertising the path.
7. **Retiring `workers.dev` is gated on evidence, not on the scrub being done.** The toggle is Theshin's (Workers & Pages → foundry → Domains → Production `workers.dev` → OFF) and is recommended **only** once a health probe reads all-green with `base: https://tailorfarms.com`. Note for future fires: `WebFetch` is **provenance-gated in unattended runs** (`PROVENANCE_REQUIRED`) — a scheduled fire generally cannot confirm the live site itself. Routing around that with `curl` to manufacture the evidence is forbidden; arm the Actions probe and report the verification as pending.
