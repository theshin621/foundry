...
2. **Cloudflare** — connect the repo to Pages once (dashboard, ~5 min): production deploys on push to `main`, previews on branches. Runtime then needs **no Cloudflare token at all** — deploy IS the git push. Enable free Web Analytics; drop the token into `lib/beacon.html`.
