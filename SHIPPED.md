...
| 003 | 2026-08-04 | [codeowners](public/003-codeowners/) | Paste a CODEOWNERS file and a PR's changed paths — see which rule wins per file, who gets requested, and which lines GitHub skips | **NOT A PASS** — 6 independent checker verdicts over 3 fix cycles; final 2 are FAIL. Matching correct (~500k differential pairs, 0 divergences) and secure; the page's own 750 ms timing promise is still false (worst 23.7 s on a 100,000-character owner token). Stopped rather than patched a 4th time — the defects share one root cause, recorded as a design constraint. | failed (unmerged) |
