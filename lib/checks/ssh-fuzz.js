...
function runCase(cfg, host, user) {
  ...
  if (live.code !== 0 || !live.out.trim()) return { skipped: true, why: (live.err.trim().split('\n')[0] || 'ssh refused') };
...
/* ── main ───────────────────────────────────────────────────────────────── */
var stats = { run: 0, skipped: 0, bad: 0 }, failures = [], derivedSeen = {};
for (var i = 0; i < N; i++) {
...
console.log(JSON.stringify({ seed: SEED, requested: N, evaluated: stats.run, skipped: stats.skipped,
  failed: stats.bad, derivedByOpenSSH: derivedSeen,
