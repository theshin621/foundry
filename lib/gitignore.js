...
(function (root, factory) {
  ...
  function compileSegment(str, nc) {
    var toks = [], i = 0, n = str.length, c, plain = true;
    /* CASE FOLDING (added 2026-08-07, checker round 1, finding A2). npm's walker
       matches case-INSENSITIVELY and always has: ignore-walk passes `nocase: true`
       to minimatch unconditionally. Verified against real npm 10.9.7 rather than
       taken from the report — a .npmignore of `dist/` + `*.log` excludes
       `Dist/inside.js` and `DEBUG.LOG`. A case-SENSITIVE matcher is therefore not
       a stricter version of npm, it is a different and wrong answer on any repo
       with a capital letter in a path.
       Folding happens ONCE, at compile: literals are lowered here and the
       candidate string is lowered once per call in test()/matchOne(), so the hot
       loop stays a plain compare with no per-character toLowerCase(). */
    if (nc) str = str.toLowerCase();
    while (i < n) {
      c = str.charAt(i);
  ...
  /* Under nocase the candidate is lowered before matching, so a class written in
     upper case would never fire. Add a lowered TWIN for every range that touches
     A-Z. Doing it before the negation is applied is what makes `[!A-Z]` come out
     right: 'a' now hits the twin, so the negation correctly rejects it. */
  function fold(items) {
    var out = items.slice(), i, it, a, b;
    for (i = 0; i < items.length; i++) {
      it = items[i]; a = it.a.toLowerCase(); b = it.b.toLowerCase();
      if (a !== it.a || b !== it.b) out.push({ a: a, b: b });
    }
    return out;
  }

  function parseClass(str, start, nc) {
    ...
    while (i < n) {
      c = str.charAt(i);
      if (c === ']') return { tok: { t: T_CLASS, neg: neg, items: nc ? fold(items) : items }, next: i + 1 };
  ...
  function parseLine(raw, lineNo, nc) {
    ...
    for (j = 0; j < parts.length; j++) {
      ...
      else segs.push(compileSegment(parts[j], nc));
    }
  ...
  function compile(text, opts) {
    ...
    for (i = 0; i < lines.length; i++) {
      ...
      r = parseLine(lines[i], i + 1, !!opts.nocase);
      if (!r) continue;
      if (r.error) { errors.push(r); continue; }
      rules.push(r);
    }
    return { rules: rules, errors: errors, lineCount: lines.length, nocase: !!opts.nocase };
  }
  ...
  function splitPath(path, nc) {
    var s = String(path);
    if (nc) s = s.toLowerCase();
    var segs = s.split('/'), i;
  ...
  function test(compiled, path, isDir, budget) {
    var segs = splitPath(path, compiled.nocase), i, r;
  ...
  /* compileOne / testRule — parse a pattern ONCE, then match it many times.
   *
   * ADDED 2026-08-07 after checker round 1, finding B1, which is worth stating in
   * full because it is the third appearance of the same defect class in this repo:
   * the old matchOne() parsed its pattern on EVERY call and only ticked the budget
   * once it reached the DP. A files[] entry of 8,180 spaces is stripped to '' by
   * parseLine's trailing-whitespace rule, so it returned early and NEVER TICKED —
   * 48,000 calls, 2,941 ms of blocked main thread, and `budget.left` still exactly
   * 60,000,000 at the end. A budget that cannot see a loop does not bound it.
   * Two changes, both needed: matchOne now ticks BEFORE it parses, and callers that
   * match one pattern against many paths must use compileOne() so the parse leaves
   * the inner loop altogether. lib/npmpack.js does the latter.
   *
   * THE GENERAL RULE, earned three times now: a metered function must charge for
   * the work it does on the way to the meter, including the work it does to decide
   * it can return early. */
  function compileOne(pattern, nc) {
    var r = parseLine(String(pattern), 0, !!nc);
    if (!r || r.error) return null;
    r.nocase = !!nc;
    return r;
  }

  function testRule(rule, path, isDir, budget) {
    if (!rule) return false;
    if (tick(budget)) return false;
    if (rule.dirOnly && !isDir) return false;
    var segs = splitPath(path, rule.nocase);
    if (!segs.length) return false;
    return pathMatch(rule.segs, segs, budget);
  }

  /* Convenience for one-off use. Charges the budget BEFORE parsing — see above. */
  function matchOne(pattern, path, isDir, budget, nc) {
    if (tick(budget, 1 + String(pattern).length)) return false;
    return testRule(compileOne(pattern, nc), path, isDir, budget);
  }
