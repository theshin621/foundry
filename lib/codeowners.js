...
(function (root, factory) {
  ...
  function resolve(parsed, path, budget) {
    ...
    if (budget && budget.exceeded) return { budgetExceeded: true, winner: null, alsoMatched: [] };
    /* hits.slice(1) allocates a copy proportional to the number of MATCHING rules, and
       "matching" is where the multiplication lives: a file where every rule matches every
       path makes this O(rules) per path on top of the metered matching. It was free.
       Charged 2026-08-06 after an independent adversary measured the surrounding
       aggregation at 15,577 ms on 100M hits against 2,105 ms on ~0 hits with the identical
       rule and path counts — the cost tracked the HIT RATE, which nothing was metering. */
    if (budget) {
      budget.left -= (hits.length >> 1) + 1;
      if (budget.left <= 0) { budget.exceeded = true; return { budgetExceeded: true, winner: null, alsoMatched: [] }; }
    }
