...
(function (root, factory) {
   ...
   *   maxLines       lines, counting a trailing newline's empty last line the way
   *                  a human counts them (i.e. "a\n" is one line, not two)
  ...
  function scan(text, limits) {
    ...
    for (i = 0; i <= n; i++) {
      ...
      line++;
      if (line > maxLines)
        return { ok: false, kind: 'lines', line: line, length: line, limit: maxLines };
