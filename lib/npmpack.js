...
(function (root, factory) {
  ...
  /* A BACKSLASH IS AN ORDINARY FILENAME CHARACTER ON THE PLATFORMS npm PUBLISHES
     FROM, and this function used to rewrite it to '/'. Checker round 1, finding A1:
     a real file named `weird\file.js` was reported as `weird/file.js`, excluded by
     a `weird/` rule, and given the reason "the directory 'weird' is excluded" —
     a directory that does not exist. Real npm ships that file untouched (verified
     against npm 10.9.7). Rewriting a separator is a guess about the user's OS
     dressed up as normalisation: it changed the file's IDENTITY and then invented
     a citation for the verdict. Only './' and stray slashes are stripped now. */
  function norm(p) {
  ...
  /* A files[] entry is ALWAYS relative to the package root — npm's packlist turns
     each entry into a root-anchored pattern. Verified against npm 10.9.7 (finding
     A3): files:["dist"], ["./dist"] and ["/dist"] all ship dist/a.js and all leave
     sub/dist/b.js OUT. So strip a leading './' and then force the leading '/' that
     makes lib/gitignore.js anchor it. */
  function normEntry(e) {
    var s = String(e == null ? '' : e).replace(/^\.\//, '');
    while (s.length > 1 && s.charAt(s.length - 1) === '/') s = s.slice(0, -1);
    if (s === '' || s === '/') return '';
  ...
  function resolve(input) {
    ...
    var user = chosenName ? gitignore.compile(chosenText, { maxRules: 5000, nocase: true }) : null;
    var hard = gitignore.compile(HARD.join('\n'), { nocase: true });
    var secrets = gitignore.compile(SECRET_PATTERNS.join('\n'), { nocase: true });

    /* `files` as a STRING is a plausible typo and npm does NOT ignore it — it
       iterates the string, so "dist" becomes the four entries d, i, s, t and the
       package ends up with almost nothing in it. Checker round 1, finding A4: the
       old Array.isArray guard failed silently and published EVERYTHING, which is
       the opposite of what npm does and the more dangerous direction to be wrong
       in. Emulate npm exactly and raise a note. */
    var filesRaw = null, filesWasString = false;
    if (pkg && Object.prototype.hasOwnProperty.call(pkg, 'files')) {
      if (Array.isArray(pkg.files)) filesRaw = pkg.files;
      else if (typeof pkg.files === 'string') { filesRaw = pkg.files.split(''); filesWasString = true; }
    }
    var filesArr = filesRaw
      ? filesRaw.filter(function (x) { return typeof x === 'string' && x !== ''; }) : null;
    var hasFilesArray = !!filesArr;

    /* Parse each files[] entry ONCE. The old code parsed every entry again for
       every path (finding B1's amplifier): 16 entries x 3,000 paths = 48,000
       parses inside a loop the budget could not see. */
    var filesRules = null, i0;
    if (hasFilesArray) {
      filesRules = [];
      for (i0 = 0; i0 < filesArr.length; i0++)
        filesRules.push({ raw: filesArr[i0], index: i0, rule: gitignore.compileOne(normEntry(filesArr[i0]), true) });
    }

    var mainTarget = (pkg && typeof pkg.main === 'string') ? norm(pkg.main) : null;
    /* npm force-includes `browser` exactly the way it force-includes main and bin
       (packlist: strict.push('!/' + browser)). Verified against npm 10.9.7 —
       browser:"b/br.js" ships even with files:["nothing"]. Finding A5. */
    var browserTarget = (pkg && typeof pkg.browser === 'string') ? norm(pkg.browser) : null;
    ...
    function filesHit(path) {
      var i, j, a = ancestorsOf(path), fr;
      for (i = 0; i < filesRules.length; i++) {
        fr = filesRules[i];
        if (!fr.rule) continue;
        if (gitignore.testRule(fr.rule, path, false, budget)) return { entry: fr.raw, index: fr.index, via: null };
        for (j = 0; j < a.length; j++) {
          if (gitignore.testRule(fr.rule, a[j], true, budget)) return { entry: fr.raw, index: fr.index, via: a[j] };
        }
    ...
    for (i = 0; i < paths.length; i++) {
      ...
      if (p === 'package.json' || isRootReadmeOrLicence(p) ||
          (mainTarget && p === mainTarget) || (browserTarget && p === browserTarget) ||
          bins.indexOf(p) !== -1) {
        ...
        row.detail = p === 'package.json' ? 'package.json'
          ...
          : (browserTarget && p === browserTarget) ? 'the "browser" entry point'
          : 'a "bin" entry point';
    ...
    if (filesWasString) {
      notes.push({ level: 'warn', text: 'package.json "files" is a STRING, not an array. npm does not ignore that — it iterates the string, so each CHARACTER becomes an entry and almost nothing is published. That is what is shown below.' });
    }
    if (hasFilesArray && (ni.present || gi.present)) {
